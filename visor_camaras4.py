...
class CameraViewerApp:
    def __init__(self, window, window_title):
        ...
        self.active_players = {}
        self.camera_frames = [] # Almacenará los frames donde se muestra el video
        self.current_page = 0
        self.total_pages = 0
        ...
        # --- Instancia de VLC ---
        self.vlc_instance = vlc.Instance("--no-xlib") #"--no-xlib" para evitar problemas en algunos sistemas
        self.is_muted = False
        ...
        # Botón de Mute
        self.mute_button = ttk.Button(top_bar, text="Mutear Audio", command=self.toggle_mute)
        ...
        # Frame para la vista en pantalla completa
        self.fullscreen_frame = tk.Frame(self.window, bg="black")
        self.fullscreen_label = ttk.Label(self.fullscreen_frame, text="", font=("Helvetica", 14, "bold"), background='black', foreground='white'); self.fullscreen_label.pack(pady=(10,5))
    ...
    def toggle_mute(self):
        """Mutea o desmutea el audio de todos los reproductores activos."""
        self.is_muted = not self.is_muted
        for player in self.active_players.values():
            player.audio_set_mute(self.is_muted)
        
        # Actualizar el texto del botón
        if self.is_muted:
            self.mute_button.config(text="Reactivar Audio")
        else:
    ...
    def update_page_view(self):
        ...
        for i in range(CAMS_PER_PAGE):
            ...
            if global_index < num_total_cameras:
                ...
                video_frame.bind("<Button-3>", lambda e, idx=global_index: self.show_context_menu(e, idx))
            else:
                video_frame.config(bg="gray80", cursor="")
    ...
    def start_stream(self, global_index, url, frame_widget):
        ...
        # Asignar el frame de Tkinter a VLC
    ...
    def exit_fullscreen(self):
        ...
        # Encontrar el frame original en la cuadrícula para devolver el video
        on_page_index = self.fullscreen_camera_index % CAMS_PER_PAGE
        original_frame = self.camera_frames[on_page_index]
        
        player = self.active_players[self.fullscreen_camera_index]
        player.set_hwnd(original_frame.winfo_id())
    ...
    def reload_stream(self, global_index):
        ...
        on_page_index = global_index % CAMS_PER_PAGE
        frame_widget = self.camera_frames[on_page_index]
        url = self.all_camera_urls[global_index]
        self.start_stream(global_index, url, frame_widget)
    ...
    # --- Métodos de Ayuda y Navegación (sin cambios mayores) ---
...
if __name__ == '__main__':
    ...
    app = CameraViewerApp(root, "Visor de Cámaras con Audio (VLC)")